...
Pattern: 0-1 Knapsack(Infinite Source)
...
Recurrent Relation:
	...
	solve(index, target) = min(
		...
		solve(index+1, target) )				// Don't take the current element.

Base condition:

	// Last element left, T >= 0
	if(index == 0)
		return T/nums[0], if T % nums[0] == 0..
...
One trick is that, whenever in any question this situation happen = INT_MAX + X(some value), due to we return INT_MAX as larger value
and then need to use few if-else condition that time instead of returing INT_MAX... use 1e9(MAX)... because it is also large number < INT_MAX.
And we can avoid those checks. At then end just check desired result is < MAX.
