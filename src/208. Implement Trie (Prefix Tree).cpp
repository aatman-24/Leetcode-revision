...
Topic : Trie
Problems : https://leetcode.com/problems/implement-trie-prefix-tree/description/?envType=problem-list-v2&envId=trie
...
Time: O(M) for each of matching function, M = size of word.
Space: O(breadth^depth) => O(26^M)
...
public:
    ...
    void insert(string word) {  // O(M), M = word.size()
        Node* node = root;
        for (char ch : word) {
